...
const val ACCEPTED_WORKFLOW_INDEX = 10000
const val REJECTED_WORKFLOW_INDEX = 10001
...
sealed class Rule(var next: String) {
    class GoTo(next: String) : Rule(next)
    class GoToIf(val op1: Operand, val op: Operator, val op2: ULong, next: String) : Rule(next)
}

sealed class OptimizedRule() {
    data class GoTo(val next: Int) : OptimizedRule()
    data class GoToIf(val op1: Operand, val op: Operator, val op2: ULong, val next: Int) : OptimizedRule()
}

fun String.toRule(): Rule {
    if (this.contains('<')) {
        var (op1, rest) = this.split('<')
        var (op2, next) = rest.split(':')
        return Rule.GoToIf(Operand.valueOf(op1.uppercase()), Operator.LessThan, op2.toULong(), next)
    } else if (this.contains('>')) {
        var (op1, rest) = this.split('>')
        var (op2, next) = rest.split(':')
        return Rule.GoToIf(Operand.valueOf(op1.uppercase()), Operator.GreaterThan, op2.toULong(), next)
    } else {
        return Rule.GoTo(this)
    }
}

data class Workflow(val name: String, val rules: List<Rule>)

data class OptimizedWorkflow(val rules: List<OptimizedRule>) {
    @OptIn(ExperimentalUnsignedTypes::class)
    fun getNext(case: ULongArray): Int {
        for (rule in rules) {
            when (rule) {
                is OptimizedRule.GoTo -> return rule.next
                is OptimizedRule.GoToIf -> when (rule.op) {
                    Operator.LessThan -> {
                        if (case[rule.op1.ordinal] < rule.op2) {
                            return rule.next; }
                    }
        ...
        throw Error("Workflow didn't terminate.")
    }
}

fun String.toWorkflow(): Workflow {
    var (name, rules, _) = this.split('{', '}')
    return Workflow(name, rules.split(',').map { it.toRule() })
}

@OptIn(ExperimentalUnsignedTypes::class)
fun String.toCase(): ULongArray {
    var parts = this.split('=', ',', '}')
    return ulongArrayOf(parts[1].toULong(), parts[3].toULong(), parts[5].toULong(), parts[7].toULong())
}

fun toOptimizedRule(rule: Rule, workflowNameToIndexMap: Map<String, Int>): OptimizedRule {
    var nextIndex = when (rule.next) {
        "A" -> ACCEPTED_WORKFLOW_INDEX
        "R" -> REJECTED_WORKFLOW_INDEX
        else -> workflowNameToIndexMap[rule.next]!!
    }

    return when (rule) {
        is Rule.GoTo -> OptimizedRule.GoTo(nextIndex)
        is Rule.GoToIf -> OptimizedRule.GoToIf(rule.op1, rule.op, rule.op2, nextIndex)
    }
}

@OptIn(ExperimentalUnsignedTypes::class)
fun toOptimizedSystem(system: System): OptimizedSystem {
    var workflowNameToIndexMap =
        system.workflows.values.mapIndexed { index, workflow -> workflow.name to index }.toMap()
    var optimizedWorkflows = system.workflows.values.map { workflow ->
        OptimizedWorkflow(workflow.rules.map {
            toOptimizedRule(
                it,
                workflowNameToIndexMap
            )
        })
    }
    return OptimizedSystem(optimizedWorkflows.toTypedArray(), workflowNameToIndexMap["in"]!!, system.cases)
}

data class System @OptIn(ExperimentalUnsignedTypes::class) constructor(
    val workflows: Map<String, Workflow>,
    val cases: List<ULongArray>
)

data class OptimizedSystem @OptIn(ExperimentalUnsignedTypes::class) constructor(
    val workflows: Array<OptimizedWorkflow>,
    val inWorkflowIndex: Int,
    val cases: List<ULongArray>
) {
    @OptIn(ExperimentalUnsignedTypes::class)
    fun isAccepted(case: ULongArray): Boolean {
        var next = inWorkflowIndex
        while (next != ACCEPTED_WORKFLOW_INDEX && next != REJECTED_WORKFLOW_INDEX) {
            next = workflows[next].getNext(case)
        }
        return next == ACCEPTED_WORKFLOW_INDEX
    }

    @OptIn(ExperimentalUnsignedTypes::class)
    fun countTotalAccepted(): ULong {
        var xs = mutableSetOf<ULong>(1u, 4000u, 4001u)
        var ms = mutableSetOf<ULong>(1u, 4000u, 4001u)
        var as_ = mutableSetOf<ULong>(1u, 4000u, 4001u)
        var ss = mutableSetOf<ULong>(1u, 4000u, 4001u)

        for (workflow in this.workflows) {
            for (rule in workflow.rules) {
                if (rule is OptimizedRule.GoToIf && rule.op == Operator.LessThan) {
                    when (rule.op1) {
                        Operand.X -> {
                            xs.add(rule.op2 - 1u); xs.add(rule.op2)
                        }

                        Operand.M -> {
                            ms.add(rule.op2 - 1u); ms.add(rule.op2)
                        }

                        Operand.A -> {
                            as_.add(rule.op2 - 1u);as_.add(rule.op2)
                        }

                        Operand.S -> {
                            ss.add(rule.op2 - 1u); ss.add(rule.op2)
                        }
                    }
                } else if (rule is OptimizedRule.GoToIf && rule.op == Operator.GreaterThan) {
                    when (rule.op1) {
                        Operand.X -> {
                            xs.add(rule.op2); xs.add(rule.op2 + 1u)
                        }

                        Operand.M -> {
                            ms.add(rule.op2); ms.add(rule.op2 + 1u)
                        }

                        Operand.A -> {
                            as_.add(rule.op2);as_.add(rule.op2 + 1u)
                        }

                        Operand.S -> {
                            ss.add(rule.op2); ss.add(rule.op2 + 1u)
                        }
                    }
                }
            }
        ...
        var xlist = xs.filter { it >= 1u && it <= 4001u }.sorted().toList()
        var mlist = ms.filter { it >= 1u && it <= 4001u }.sorted().toList()
        var alist = as_.filter { it >= 1u && it <= 4001u }.sorted().toList()
        var slist = ss.filter { it >= 1u && it <= 4001u }.sorted().toList()


        var totalAccepted = IntStream.range(0, xlist.size - 1).parallel().mapToObj<ULong>(fun(xi: Int): ULong {
            var partialSumForXi: ULong = 0u
            var case = ulongArrayOf(0u, 0u, 0u, 0u)
            var x = xlist[xi]
            println("[multithreaded] x=$x")
            case[0] = xlist[xi]
            for (mi in 0..<mlist.size - 1) {
                case[1] = mlist[mi]
                for (ai in 0..<alist.size - 1) {
                    case[2] = alist[ai]
                    for (si in 0..<slist.size - 1) {
                        case[3] = slist[si]
                        if (this.isAccepted(case)) {
                            partialSumForXi +=
                                (xlist[xi + 1] - xlist[xi]) *
                                    (mlist[mi + 1] - mlist[mi]) *
                                    (alist[ai + 1] - alist[ai]) *
...
@OptIn(ExperimentalUnsignedTypes::class)
fun String.toSystem(): System {
    var lines = this.lines()
    return System(
        lines.takeWhile { p -> p.isNotEmpty() }.map { p -> p.toWorkflow() }.associateBy { it.name },
        lines.takeLastWhile { p -> p.isNotEmpty() }.map { p -> p.toCase() }.toList()
    )
}
...
{x=2127,m=1623,a=2188,s=1013}"""
    ...
    var testSystem = toOptimizedSystem(testInput.toSystem())

    var testPart1 = testSystem.cases.filter { testSystem.isAccepted(it) }.sumOf { i -> i.sum() }
    println("Part 1 (test input): $testPart1")
    var testPart2 = testSystem.countTotalAccepted()
    println("Part 2 (test input): $testPart2")

    var input = """cq{a<289:R,a<499:A,m>826:A,A}
...
{x=1478,m=268,a=745,s=270}"""
    ...
    var system = toOptimizedSystem(input.toSystem())
    var part1 = system.cases.filter { system.isAccepted(it) }.sumOf { it.sum() }
    println("Part 1 (input): $part1")
    var part2 = system.countTotalAccepted()
